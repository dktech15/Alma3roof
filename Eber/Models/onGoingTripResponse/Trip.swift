...
class Trip: Model {
    ...
    var is_ride_share = false
    var is_otp_verification = false
    var confirmation_code = 0
    var bids: [Bids] = []
	...
	init(fromDictionary dictionary: [String:Any]) {
        ...
        is_ride_share = (dictionary["is_ride_share"] as? Bool) ?? false
        is_otp_verification = (dictionary["is_otp_verification"] as? Bool) ?? false
        confirmation_code = (dictionary["confirmation_code"] as? Int) ?? 0
        
        
        if let value = dictionary["split_payment_users"] as? [[String:Any]] {
            split_payment_users.removeAll()
            for obj in value {
                split_payment_users.append(SearchUser(dictionary: obj))
            }
        }
        ...
        self.bids.removeAll()
        if let bids = dictionary["bids"] as? [[String:Any]] {
            for bid in bids {
                self.bids.append(Bids(dictionary: bid as NSDictionary))
            }
        }
...
class DestinationAddresses: Model {
    var location : [Double]!
    var address : String!
    
    init(fromDictionary dictionary: [String:Any]) {
        
        self.location = (dictionary["location"] as? [Double]) ?? [0.0,0.0]
